...
package fr.mathis.tourhanoipro.listener;
...
public class SwipeToDismissTouchListener implements RecyclerView.OnItemTouchListener {
	...
	private void cancel() {
		...
		animate(mSwipeView).translationX(0).alpha(1).setDuration(mAnimationTime).setListener(null);
	...
	private void up(MotionEvent motionEvent) {
		...
		if (dismiss) {
			...
			++mDismissCount;
			animate(mSwipeView).translationX(dismissRight ? mViewWidth : -mViewWidth).alpha(0).setDuration(mAnimationTime);
			...
			swipeViewCopy.postDelayed(new Runnable() {
				...
				public void run() {
					...
					ObjectAnimator translationAnim = ObjectAnimator.ofFloat(swipeViewCopy, "translationX", 0, 0);
	...
	private boolean move(MotionEvent motionEvent) {
		...
		if (mSwiping) {
			...
			ObjectAnimator translationAnim = ObjectAnimator.ofFloat(mSwipeView, "translationX", 0, deltaX - mSwipingSlop);
			translationAnim.setDuration(0);
			translationAnim.start();

			AlphaAnimation alphaAnimation = new AlphaAnimation(Math.max(0f, Math.min(1f, 1f - 2f * Math.abs(deltaX) / mViewWidth)), Math.max(0f, Math.min(1f, 1f - 2f * Math.abs(deltaX) / mViewWidth)));
